...
import { depositToBalance, releaseBidToBalance } from "services/balance-transaction";

import { ApiErrorCode, AppController } from "types/app";
import { DepositError } from "types/services";
import {
  AuthRequest,
  BalanceTransactionResponse, BidReleaseRequestBody,
  DepositRequestBody
} from "types/apis";
...
class AccountBalanceCtrl implements AppController {
  ...
  releaseBidApi = async (
    req: AuthRequest<never, never, BidReleaseRequestBody>,
    res: Response<BalanceTransactionResponse>,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const {
        errCode,
        data
      } = await releaseBidToBalance(userId, itemId);
  ...
  initRoute(): Router {
    ...
    router.post("/bid/release", validateRequestScope(["super-admin"]), this.releaseBidApi);
    return router;
  }
